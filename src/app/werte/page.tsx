...
export default function WertePage() {
  return (
    <>
        ...
        <div className="container-max">
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.slice(0, 6).map((value) => (
              <StaggerItem key={value.number}>
                <div className="relative rounded-2xl glass-card p-8 hover:border-[#c9a84c]/40 transition-all duration-300 group hover:-translate-y-2 overflow-hidden h-full">
                  <div className="absolute top-0 right-0 w-24 h-24 rounded-full bg-gradient-to-br opacity-5 group-hover:opacity-10 transition-opacity duration-300 -translate-y-8 translate-x-8"
                    style={{ background: `linear-gradient(135deg, ${value.color.includes('c9a84c') ? '#c9a84c' : value.color.includes('e8455a') ? '#e8455a' : value.color.includes('3060b0') ? '#3060b0' : value.color.includes('20a060') ? '#20a060' : value.color.includes('8040c0') ? '#8040c0' : '#0080c0'}, transparent)` }}
                  />
          ...
          <AnimatedChild delay={0.3} className="mt-6">
            <div className="relative rounded-2xl overflow-hidden">
              ...
              <div className="relative glass-card rounded-2xl p-10 md:p-14 hover:border-[#c9a84c]/40 transition-all duration-300 group">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                  ...
                  <div className="md:col-span-2">
                    <h3 className="text-3xl font-black text-white mb-4">{values[6].title}</h3>
                    <p className="text-gray-300 text-lg leading-relaxed">{values[6].text}</p>
      ...
      {/* Closing statement */}
      <section className="py-24 bg-[#060c18] text-center">
        <div className="container-max">
          <AnimatedChild>
